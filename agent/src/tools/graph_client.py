"""Neptune OpenCypher client with IAM SigV4 auth for the Agent.

Uses boto3 neptunedata client over HTTPS. Thread-safe — boto3 manages
its own connection pool. No persistent WebSocket connections needed.
"""
...
from src.config import NEPTUNE_ENDPOINT
...
def get_client():
    """Return a cached boto3 neptunedata client (thread-safe)."""
    global _client
    if _client is None:
