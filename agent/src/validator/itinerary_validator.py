...
def validate_itinerary(output: PlanningOutput) -> ValidationResult:
    ...
    # Graph-grounded city-scope checks are run inside the validate node
    # via MCP (Lambda → Neptune). They're intentionally not invoked here so
    # this function stays free of Neptune connectivity assumptions.
...
def issues_from_city_scope_response(
    output: PlanningOutput, payload: dict | None
) -> list[Issue]:
    """Convert a validate_city_scope MCP response payload into Issue objects.

    payload format::

        {"issues": [{"day", "name", "actual_cities", "expected_cities"}],
         "missing": [{"day", "name"}]}
    """
    ...
    for m in payload.get("missing", []) or []:
        try:
            day_num = int(m.get("day"))
        except (TypeError, ValueError):
            continue
        name = m.get("name", "")
        # Find this day's cities to suggest a recommend_attractions hint
        suggestion_city = ""
        for d in output.itinerary:
            if d.day == day_num:
                suggestion_city = next(
                    (c.strip() for c in (d.cities or "").split(",") if c.strip()),
                    "",
                )
                break
        issues.append(
            Issue(
                ...
                suggestion=(
                    f"recommend_attractions(city={suggestion_city}) 결과의 "
    ...
    for it in payload.get("issues", []) or []:
        try:
            day_num = int(it.get("day"))
        except (TypeError, ValueError):
            continue
        name = it.get("name", "")
        actual = it.get("actual_cities") or []
        expected = it.get("expected_cities") or []
        actual_str = ", ".join(actual)
        day_str = ", ".join(expected)
        issues.append(
            Issue(
                severity=Severity.ERROR,
                day=day_num,
