...
[
  ...
  {"name":"upsert_trend","description":"Upsert a Trend vertex in Neptune. Creates if not exists, updates if exists.","inputSchema":{"type":"object","properties":{"title":{"type":"string","description":"Trend title."},"type":{"type":"string","description":"Trend type (e.g. drama, variety, festival, food)."},"source":{"type":"string","description":"Source (e.g. youtube, naver, google_trends, news)."},"date":{"type":"string","description":"ISO date string."},"virality_score":{"type":"integer","description":"Virality score 0-100."},"decay_rate":{"type":"number","description":"Decay rate 0.0-1.0."},"keywords":{"type":"array","items":{"type":"string"},"description":"Related keywords."}},"required":["title","type","source","date","virality_score","decay_rate"]}},
  {"name":"upsert_trend_spot","description":"Upsert a TrendSpot vertex in Neptune.","inputSchema":{"type":"object","properties":{"name":{"type":"string","description":"Spot name."},"description":{"type":"string"},"category":{"type":"string"},"lat":{"type":"number"},"lng":{"type":"number"},"photo_worthy":{"type":"boolean"}},"required":["name"]}},
  {"name":"link_trend_to_spot","description":"Link a Trend to a TrendSpot with optional City connection.","inputSchema":{"type":"object","properties":{"trend_title":{"type":"string"},"trend_source":{"type":"string"},"spot_name":{"type":"string"},"edge_label":{"type":"string","description":"FILMED_AT or FEATURES"},"city_name":{"type":"string","description":"City to link TrendSpot via LOCATED_IN."}},"required":["trend_title","trend_source","spot_name"]}},
  {"name":"invalidate_cache","description":"Delete cached MCP tool results from Valkey. Use after trend updates or graph rebuilds.","inputSchema":{"type":"object","properties":{"tool_pattern":{"type":"string","description":"Tool name to invalidate e.g. get_trends"},"flush_all":{"type":"boolean","description":"If true delete ALL mcp keys"}},"required":[]}}
]
TOOLS
)
        }
